...
TEMPLATES = [
    ...
    },
    {
        ...
        "note": "Turbo: CFG 1 (guidance off), 8 steps (4 minimum). Raw/Base: ~52 steps, CFG 3.5 — for LoRA training and variation re-rolls. Sigma Shift 1.15; 2048-native. Internal text refiner strips NSFW terms. Native SwarmUI support: grab BASE/TURBO from HF 'Comfy-Org/Krea-2' into the diffusion_models folder (Image Gen → Download model). Lower-VRAM GGUF build instead: HF 'realrebelai/KREA-2_GGUFs' (BASE/TURBO, diffusion_models) plus its Qwen3-VL-4B-Instruct GGUF text encoder (clip folder) — SwarmUI auto-offers to install city96/ComfyUI-GGUF the first time it sees a .gguf file, but that release doesn't parse Krea 2's ops yet (city96/ComfyUI-GGUF#464), so swap it for the RealRebelAI/ComfyUI-GGUF_KREA-2 fork in SwarmUI's ComfyUI custom_nodes folder instead (same node/class names, drop-in replacement).",
