...
TEMPLATES = [
    ...
    {
        "id": "krea2", "label": "Krea 2 Turbo (sentences, 8 steps)",
        "match": ["krea"],
        "prefix": "",
        "suffix": "",
        ...
        "note": "Turbo: CFG 1 (guidance off), 8 steps (4 minimum). Raw/Base: ~52 steps, CFG 3.5 — for LoRA training and variation re-rolls. Sigma Shift 1.15; 2048-native. Internal text refiner strips NSFW terms. Unsloth backend: select Krea 2 Turbo GGUF natively in Studio's Model Hub — no custom nodes, forks, or folder juggling. NOTE: the Unsloth /v1 images endpoint ignores negative prompts and steps/guidance for turbo models. Legacy SwarmUI backend: native support via HF 'Comfy-Org/Krea-2' into the diffusion_models folder (Image Gen → Download model), or the lower-VRAM GGUF path via the Image Gen tab's 'Krea 2 GGUF Quick Setup' panel plus running install-comfyui-gguf-krea2.sh (repo root) once on the SwarmUI host to install the RealRebelAI/ComfyUI-GGUF_KREA-2 fork (city96/ComfyUI-GGUF doesn't parse Krea 2's ops yet — city96/ComfyUI-GGUF#464).",
