...
async def _chat_kwargs(extra_options: dict = None, think: bool = False, model: str = "") -> dict:
    ...
    For raw GGUF models pulled via the hf.co/ prefix (including the
    unsloth/gemma-4-26B-A4B-it-GGUF family), Ollama won't tag them with
    the "thinking" capability even though Gemma 4 supports it natively.
    The _model_supports_thinking check is therefore bypassed for known
    thinking-capable model id prefixes so that think=True is honoured
    rather than silently downgraded to False."""
    if think and model and not _is_known_thinking_model(model):
        if not await _model_supports_thinking(model):
            think = False
    kwargs = {"think": think}
...
# Model-id prefixes that are known to support thinking even when Ollama's
# /api/show doesn't list the "thinking" capability (e.g. raw GGUF pulls via
# the hf.co/ prefix that lack a Modelfile with capability declarations).
# Checked BEFORE the /api/show round-trip in _chat_kwargs so that think=True
# is honoured for these models rather than silently downgraded to False.
_KNOWN_THINKING_PREFIXES = (
    "hf.co/unsloth/gemma-4-",
    "hf.co/noctrex/gemma-4-",
    "gemma4:",
)


def _is_known_thinking_model(model: str) -> bool:
    return any(model.startswith(p) for p in _KNOWN_THINKING_PREFIXES)
...
async def summarize_session_from_facts(facts: list[str], model: str = "", extra_instructions: str = "", think: bool = True) -> str:
    ...
    opts = dict(_thinking_num_predict_override(think))
    reserve = _CONTEXT_FIT_RESERVED_TOKENS + (_THINKING_HEADROOM_TOKENS if "num_predict" in opts else 0)
    opts.update(_ctx_override_if_needed(system + facts_text, reserve))
    return await generate_chat(
        [{"role": "user", "content": facts_text}], system=system, model=model,
        options=opts or None, think=think,
    )
