...
async def api_facts_bulk(request: Request, db: Session = Depends(get_db), active_world: str = Cookie(None)):
    ...
    # Existing keys for the whole WORLD (not just this session): a fact may
    # legitimately live on no session or a different one, and "the same
    # sentence is already logged somewhere in this world" is the duplicate
    # condition a GM actually cares about when confirming a parse.
    seen_keys = {
        _ai_module._normalized_fact_key(content)
        for (content,) in db.query(Fact.content).filter(Fact.world_id == world.id).all()
    }
    created = []
