...
def enabled_instructions_text(db: Session, world_id: int) -> str:
    """Every enabled AiInstruction for this world, oldest first, each
    under its own "## title" heading — or "" when there are none enabled
    (a world that's never used this feature, or has switched every
    uploaded document off, builds exactly the system prompt it always
    has). Callers append this to their own surface's existing system
    prompt/grounding clause, never replace it — see each of the four call
    sites (app.main.ai_chat_page, app.main.player_ai_chat's
    ai_chat_player.html template context, app.main.detail()'s
    entities/detail.html template context, app.routers.chronicler.
    build_chronicler_system_prompt)."""
    rows = (
        db.query(AiInstruction)
        .filter(AiInstruction.world_id == world_id, AiInstruction.enabled.is_(True))
        .order_by(AiInstruction.created_at)
        .all()
    )
