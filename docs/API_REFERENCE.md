...
| POST | `/worlds/{world_id}/members/{user_id}/reset-password` | GM | Generates a one-time temporary password for a locked-out player/assistant and re-renders the edit page with it shown exactly once (never emailed — this app has no outbound mail; the GM relays it out-of-band). Also bumps `User.session_version` and clears `TrustedDevice` rows for that account, same invalidation as the self-service `/account/password` change. See also the `GM_PASSWORD_RESET` env var (README.md → Environment Variables) for the GM's own account recovery. |
| POST | `/worlds/{world_id}/members/{user_id}/clear-2fa` | GM | Clears a member's TOTP enrollment, backup-code hashes, and trusted devices (and bumps `session_version`) — the recovery path for a player who lost their authenticator AND backup codes, where a password reset alone can't help since login still stalls at `/login/2fa`. The member then logs in with just their password and re-enrolls two-step from their account page. |
...
| GET | `/entity/{entity_id}` | Player | Entity detail page: body, notes, links to other entities, image gallery. Any other entity's name OR `Entity.aliases` (comma-separated alternate/short names, e.g. "Vosk, Edmund Vosk" for a full name of "Hunter Edmund Vosk, the Greyfather") mentioned in the body or a note is automatically wikilinked (`rendering.autolink_entities`) — no manual `[Name](url)` needed; only entities the current viewer can already see are eligible, and an entity never links to itself. |
...
| POST | `/api/ai/world-context-player` | GM* | Player-safe counterpart used by `/ai-chat` — same retrieval as `/api/ai/world-context-smart`, but filtered to entities/notes `visible_to_players` (or specifically shared with the caller), same rule `/kind/{kind}`/`/entity/{id}` enforce. Body `{query, limit?, notes_limit?}`; `limit`/`notes_limit` default to the active world's `ai_chat_rag_entity_limit`/`ai_chat_rag_notes_limit` (set on `/worlds/{id}/edit`, NULL by default) or a built-in 15/3, and are clamped to `[0, 25]`/`[0, 10]` regardless of source. Same `players_can_ask_ai`/`players_can_use_ai_chat` gate as `/api/ai/stream`. |
| POST | `/api/ai/entity-context` | GM* | Per-question relevant excerpt of ONE entity's own body, for the entity detail page's "Ask AI about {entity}" panel — body `{entity_id, query}`. Uses keyword-relevance section search (same as `rules_context`/`priority_entities_context`) instead of a blind prefix slice, so a large heading-structured entity's actually-relevant section reaches the model regardless of where in the document it sits. Same entity-visibility check as the detail page itself (404 if this viewer couldn't see the entity), `[gmonly]` stripped for a non-GM viewer, same `players_can_ask_ai`/`players_can_use_ai_chat` gate as `/api/ai/stream`. |
| GET | `/npc-talk` | GM* | The "Talk to NPCs" page — persistent, per-entity in-character conversations (`ChatSession` rows with `surface="npc"`, one per user × entity × world, so every participant has their own private thread with each NPC). Picker lists only entities the caller can already see; same `players_can_ask_ai`/`players_can_use_ai_chat` gate as the AI surfaces. `?entity={id}` opens a specific NPC. |
| GET | `/api/npc-talk/{entity_id}/history` | GM* | This caller's saved conversation with that entity (404 if the entity isn't visible to them). |
| DELETE | `/api/npc-talk/{entity_id}/history` | GM* | Forgets this caller's conversation with that entity (restart). |
| GET | `/api/npc-talk/{entity_id}/export.md` | GM* | Downloads this caller's conversation as a markdown transcript. |
| POST | `/api/npc-talk/{entity_id}/stream` | GM* | Sends one message and streams the NPC's in-character reply as SSE (same wire format as `/api/ai/stream`). The model prompt — roleplay directive + the NPC's identity (summary, body prefix, custom fields, visible notes/relations) + standing AI instructions — is composed server-side under the caller's own visibility rules with `[gmonly]` stripped for non-GMs; history comes from the DB, not the request. Body `{message, think?, model?, use_rag?}` — `model`/`use_rag` are GM-only (ignored for non-GMs); both turns persist on a clean finish only, so a failure sentinel never becomes a saved reply. |
...
| POST | `/api/ai/entities-from-text-batch` | GM / Assistant | Turns one passage of text (session notes, a homebrew doc) into MULTIPLE classified drafts in one call — `{"entities": [...], "player_characters": [...]}`, both possibly empty. Draft only; POST the reviewed subset as an `{"imports": [...]}` batch to `/api/import/execute` (`kind=batch`). |
...
| POST | `/api/ai/assist` | GM / Assistant | One shared AI-assist operation (improve/deslop/expand/summarize/analyze/suggest/translate/custom/table_entries/...) on editor content — the ✨ panel every edit form embeds (app/ai_assist.py). `deslop` strips AI-writing tics (banned filler words, "not X, it's Y" contrasts, fake-profound kickers, etc. — ported from github.com/petergyang/no-ai-slop) while preserving voice and facts. Input-capped at 60k chars. |
...
| POST | `/api/ai/stream` | GM* | Streaming chat completion (SSE) — GM always; a player may if the active world's `players_can_ask_ai` is on. Accepts a per-request `options` (temperature/top_p/etc, clamped) and a `surface` for per-surface default-model fallback; the SSE stream leads with a `note` event if the requested model had to be fuzzy-matched to an available one. A generation failure (empty model response, an Ollama error, an unexpected exception) arrives as its own `error` event, distinct from `token`/`thinking` — clients must never treat it as real reply text to display or save. |
...
| POST | `/api/ai/imagegen/model-templates/custom` | GM | Save (upsert by label) one of this world's OWN model templates — current prompt as example, negative, steps/CFG/sampler/scheduler, optional match keywords for the suggested-marker. |
| DELETE | `/api/ai/imagegen/model-templates/custom/{template_id}` | GM | Delete one of this world's custom model templates. |
...
| GET | `/admin/diagnostics/events` | GM | The diagnostics watchdog's full journal (`events.log`) as plain text — one line per startup/shutdown/loop-stall/dump/pool event, the incident-reconstruction file for "the site stopped responding" reports. Also rendered as a tail in Settings → Diagnostics. |
| GET | `/admin/diagnostics/dump/{name}` | GM | One watchdog wedge-dump (all thread stacks, the asyncio task snapshot, DB pool status) as plain text. `name` is pattern-validated against the dump filename format and containment-checked inside the diagnostics dir; anything else 404s. |
...
| POST | `/api/knowledge/suggest-relations` | GM | AI: scans the active world's matched vault notes for entity relationships their prose states but `/api/knowledge/sync`'s `[[wikilink]]`/frontmatter pass didn't capture — returned for review as `{suggestions: [...]}`, **not written to the DB**. Optional body `{model?}`. |
| POST | `/api/knowledge/relations/bulk` | GM | The relation-suggestion review UI's Confirm & Save action — body `{relations: [{source_id, target_id, relation}, ...]}`. Both ids are verified to belong to the active world; duplicates (already-confirmed or repeated in the payload) are skipped. Response is `{created, skipped_duplicates}`. |
| POST | `/worlds/{world_id}/ai-instructions/import` | GM | Uploads a `.md`/`.markdown`/`.txt` file as a new `AiInstruction` — standing behavior instructions appended to every AI Chat/Ask AI/Chronicler system prompt for this world (set via Settings). GM-only (`applies_to_players=false`) unless the optional `applies_to_players` checkbox is set. |
| POST | `/worlds/{world_id}/ai-instructions/{id}/toggle` | GM | Enables/disables one `AiInstruction` without deleting it. |
| POST | `/worlds/{world_id}/ai-instructions/{id}/toggle-players` | GM | Flips whether this instruction's text is ever composed into a player-reachable prompt — opt-in, since an instruction's own text is often the spoiler it exists to guard. |
