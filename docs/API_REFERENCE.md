...
| GET | `/entity/{entity_id}` | Player | Entity detail page: body, notes, links to other entities, image gallery. Any other entity's name mentioned in the body or a note is automatically wikilinked (`rendering.autolink_entities`) — no manual `[Name](url)` needed; only entities the current viewer can already see are eligible, and an entity never links to itself. |
