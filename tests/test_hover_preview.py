...
def test_hover_preview_config_defaults(client, seed):
    ...
    assert r.status_code == 200
    assert r.json() == {"enabled": True, "delay_ms": 5000}
...
def test_settings_roundtrip_persists_hover_preview(client, seed):
    ...
    r = client.get("/api/hover-preview/config")
    assert r.json() == {"enabled": True, "delay_ms": 2500}
...
def test_delay_field_has_no_native_min_max_constraint(client, seed):
    """Regression guard for a real bug caught during manual verification:
    an HTML5 min/max attribute on this field, combined with a stored value
    ever outside that range (only reachable via direct DB access today, but
    cheap to guard against), makes the browser silently refuse to submit
    the *entire* Options form — no error, no console message, the Save
    button just stops doing anything. Clamping happens server-side in
    settings_save already; the client-side constraint added nothing but
    this footgun, so it should never come back."""
    login(client, seed.gm.email, GM_PASSWORD)
    db = SessionLocal()
    try:
        ...
        db.close()
    r = client.get("/settings")
    assert 'id="hover-preview-delay"' in r.text
    input_tag = r.text[r.text.index('id="hover-preview-delay"') - 80 : r.text.index('id="hover-preview-delay"') + 200]
