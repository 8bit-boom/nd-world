...
def test_heals_pre_ollama_server_env_schema(tmp_path, monkeypatch):
    ...
    assert {
        ...
        "ollama_model_overrides_json", "ollama_gpu_preset",
    } <= settings_cols
    ...
    try:
        ...
        assert s.ollama_gpu_preset == "" or s.ollama_gpu_preset is None
    finally:
        db.close()
...
def test_heals_pre_font_settings_worlds_schema(tmp_path, monkeypatch):
    """A worlds table predating font/font_size (the simple, no-JSON-file
    per-world font controls on the World edit page) must heal onto NULL/100
    defaults — the app default font and 100% size — not break loading an
    existing world."""
    db_path = tmp_path / "pre_font_settings.db"
    engine = create_engine(f"sqlite:///{db_path}", connect_args={"check_same_thread": False})
    SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
    ...
    with engine.begin() as conn:
        world_cols = {r[1] for r in conn.execute(text("PRAGMA table_info(worlds)")).fetchall()}
    assert {"font", "font_size"} <= world_cols

    db = SessionLocal()
    try:
        w = db.get(World, 1)
        assert w.font is None
        assert w.font_size == 100
    finally:
        db.close()
